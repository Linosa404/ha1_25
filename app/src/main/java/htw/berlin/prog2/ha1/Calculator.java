...
public class Calculator {
    ...
    public void pressUnaryOperationKey(String operation) {
        ...
        if (operation == "%" && latestOperation == "+") {
            result = latestValue + (latestValue / Double.parseDouble(screen));
        } else {
            latestValue = Double.parseDouble(screen);
    ...
    public void pressEqualsKey() {
        // Wenn keine Operation gesetzt ist, wird nichts ausgeführt
        if (latestOperation.isEmpty())
            return;

        // Bildschirmwert als aktuellen Operanden lesen
        double currentScreenValue = Double.parseDouble(screen);

        // Berechnen des Ergebnisses auf Basis der letzten Operation und des aktuellen
        // Operanden
        var result = switch (latestOperation) {
            case "+" -> latestValue + currentScreenValue;
            case "-" -> latestValue - currentScreenValue;
            case "x" -> latestValue * currentScreenValue;
            case "/" -> currentScreenValue == 0 ? Double.POSITIVE_INFINITY : latestValue / currentScreenValue;
            case "%" -> currentScreenValue;
            default -> throw new IllegalArgumentException();
        };

        // Anzeige "Error" im Fall einer Division durch Null
        ...
        // Letzter Operand wird gespeichert, um die Berechnung bei erneutem Drücken der
        // `=`-Taste zu wiederholen
